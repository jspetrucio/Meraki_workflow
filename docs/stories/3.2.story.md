...
- [ ] **T1: Build agent system prompts** (AC: 1)
  - [ ] T1.1: Create `scripts/agent_prompts.py` module
  - [ ] T1.2: Load base prompts from `.claude/agents/network-analyst.md`, `.claude/agents/meraki-specialist.md`, `.claude/agents/workflow-creator.md`
  - [ ] T1.3: Append standard instructions: "You have access to the following tools. Use them to fulfill user requests. Always explain what you're doing before executing."
  - [ ] T1.4: Append safety instructions: "For config changes, explain the impact and ask for confirmation. Always backup before modifying."
  - [ ] T1.5: Implement `build_system_prompt(agent_name: str, context: NetworkContext) -> str`

- [ ] **T2: Implement dynamic network context injection** (AC: 2)
  - [ ] T2.1: Define `NetworkContext` dataclass: org_name, org_id, network_count, device_count, profile_name
  - [ ] T2.2: Implement `get_network_context(profile: str) -> NetworkContext` — calls existing auth/api modules
  - [ ] T2.3: Inject context into system prompt: "You are managing the '{org_name}' organization with {network_count} networks and {device_count} devices."
  - [ ] T2.4: Cache context for 5 minutes (avoid repeated API calls)

- [ ] **T3: Define tool schemas for all agent functions** (AC: 3, 4)
  - [ ] T3.1: Create LLM function-calling tool definitions for network-analyst functions
  - [ ] T3.2: Create tool definitions for meraki-specialist functions
  - [ ] T3.3: Create tool definitions for workflow-creator functions
  - [ ] T3.4: Each tool definition includes: name, description, parameter JSON schema
  - [ ] T3.5: Store tool definitions in `AGENT_TOOLS` dict keyed by agent name
  - [ ] T3.6: Implement `get_agent_tools(agent_name: str) -> list[dict]`

- [ ] **T4: Implement confirmation detection** (AC: 5)
  - [ ] T4.1: Classify each tool call as: safe (read-only), moderate (reversible), dangerous (disruptive)
  - [ ] T4.2: Safe: `full_discovery`, `discover_*`, `find_issues`, `list_*`, `generate_*_report`
  - [ ] T4.3: Moderate: `configure_ssid`, `create_vlan`, `update_vlan`
  - [ ] T4.4: Dangerous: `add_firewall_rule`, `remove_firewall_rule`, `add_switch_acl`, `delete_vlan`
  - [ ] T4.5: Return classification with tool call for WebSocket confirmation flow

- [ ] **T5: Implement conversation context management** (AC: 7)
  - [ ] T5.1: Maintain message history per ChatSession (from Story 1.2 data models)
  - [ ] T5.2: Trim to last 20 messages before sending to LLM
  - [ ] T5.3: Include tool call results in conversation history
  - [ ] T5.4: Reset context on profile switch or explicit "new conversation"

- [ ] **T6: Integrate with Agent Router** (AC: all)
  - [ ] T6.1: Update Agent Router (Story 1.3) to use `build_system_prompt()` when dispatching
  - [ ] T6.2: Pass `get_agent_tools(agent_name)` to AI Engine in chat_completion call
  - [ ] T6.3: Handle tool_calls from LLM response → execute → return results → continue conversation

- [ ] **T7: Write unit tests**
  - [ ] T7.1: Test prompt building for each agent type
  - [ ] T7.2: Test network context injection
  - [ ] T7.3: Test tool schema generation for all functions
  - [ ] T7.4: Test tool call safety classification
  - [ ] T7.5: Test conversation trimming (>20 messages)
  - [ ] T7.6: Test multi-provider prompt compatibility
  - [ ] T7.7: Ensure all tests pass: `pytest tests/test_agent_prompts.py -v`
