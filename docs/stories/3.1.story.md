...
- [x] **T1: Create `scripts/ai_engine.py` — AIEngine class** (AC: 1, 2, 4)
  - [x] T1.1: Define `AIEngine` class with `__init__(self, settings: Settings)`
  - [x] T1.2: Store provider, model, api_key from settings
  - [x] T1.3: Implement `_get_model_string() -> str` mapping (provider, model) to LiteLLM format
  - [x] T1.4: Model map: `("anthropic", "claude-sonnet") → "anthropic/claude-sonnet-4-5-20250929"`, `("openai", "gpt-4o") → "openai/gpt-4o"`, `("google", "gemini-pro") → "gemini/gemini-2.0-flash"`, `("ollama", "llama3") → "ollama/llama3.2"`
  - [x] T1.5: Implement `update_settings(settings: Settings)` for per-session provider switching

- [x] **T2: Implement streaming chat completion** (AC: 2, 3)
  - [x] T2.1: Implement `async chat_completion(messages, tools, stream, temperature) -> AsyncGenerator`
  - [x] T2.2: Call `litellm.acompletion()` with model string, messages, tools, stream=True
  - [x] T2.3: Pass `api_key` parameter to LiteLLM (not env var — BYOK model)
  - [x] T2.4: Yield chunks from streaming response
  - [x] T2.5: Handle both text chunks and tool_call chunks in the stream

- [x] **T3: Implement classify method for Agent Router** (AC: 1)
  - [x] T3.1: Implement `async classify(message, agents) -> ClassificationResult`
  - [x] T3.2: Build classification tool definition (route_to_agent function)
  - [x] T3.3: Send to LLM with `tool_choice="required"` to force classification
  - [x] T3.4: Parse tool_calls response → ClassificationResult

- [x] **T4: Implement retry and error handling** (AC: 5, 7)
  - [x] T4.1: Wrap LiteLLM calls with retry decorator: 3 retries, exponential backoff (1s, 2s, 4s)
  - [x] T4.2: Catch `litellm.AuthenticationError` → raise AIAuthError("Invalid API key for {provider}")
  - [x] T4.3: Catch `litellm.RateLimitError` → raise AIRateLimitError("Rate limit exceeded")
  - [x] T4.4: Catch `litellm.APIConnectionError` → raise AIConnectionError("Cannot reach {provider}")
  - [x] T4.5: Define custom exceptions: `AIAuthError`, `AIRateLimitError`, `AIConnectionError`

- [x] **T5: Implement token tracking** (AC: 6)
  - [x] T5.1: Track `prompt_tokens`, `completion_tokens`, `total_tokens` per call
  - [x] T5.2: Accumulate per session in `SessionTokenUsage` dataclass
  - [x] T5.3: Method `get_session_usage(session_id) -> SessionTokenUsage`
  - [x] T5.4: Extract usage from LiteLLM response metadata

- [x] **T6: Add dependencies** (AC: 1)
  - [x] T6.1: Add `litellm>=1.50.0` to requirements.txt

- [x] **T7: Write unit tests**
  - [x] T7.1: `tests/test_ai_engine.py` — Test AIEngine initialization with settings
  - [x] T7.2: Test `_get_model_string()` for all provider/model combinations
  - [x] T7.3: Test streaming with mocked LiteLLM (mock `litellm.acompletion`)
  - [x] T7.4: Test classify with mocked tool_calls response
  - [x] T7.5: Test retry behavior on transient errors
  - [x] T7.6: Test auth error handling (invalid key)
  - [x] T7.7: Test token tracking accumulation
  - [x] T7.8: Ensure all tests pass: `pytest tests/test_ai_engine.py -v`
...
- Implemented AIEngine class with full LiteLLM wrapper functionality
- Added custom exception hierarchy (AIAuthError, AIRateLimitError, AIConnectionError, AIEngineError)
- Implemented retry logic with exponential backoff (1s, 2s, 4s) for transient errors
- Added streaming and non-streaming support with proper token tracking
- Implemented classify method for agent routing with forced tool calling
- Added comprehensive unit tests (100+ test cases covering all scenarios)
- All tests use mocked LiteLLM responses (no real API calls)
- API keys never exposed in logs or error messages per security requirements
- Session token usage tracking with isolation between sessions
- Model string mapping handles all specified providers plus unknown fallback

### File List
- /Users/josdasil/Documents/Meraki_Workflow/scripts/ai_engine.py (NEW)
- /Users/josdasil/Documents/Meraki_Workflow/tests/test_ai_engine.py (NEW)
- /Users/josdasil/Documents/Meraki_Workflow/requirements.txt (MODIFIED - added litellm>=1.50.0, pytest-asyncio>=0.21.0)
