...
- Task executor uses asyncio.Event gate for WebSocket confirmation flow
- Feature flag `use_modular_tasks` controls legacy vs modular routing
- All task YAML files live under `tasks/` with YAML frontmatter format
- FUNCTION_REGISTRY in agent_router.py maps tool names to Python functions

## File Locations

| File | Description |
|------|-------------|
| `scripts/task_executor.py` | Core task execution engine |
| `scripts/task_registry.py` | YAML parser and task matching |
| `scripts/task_models.py` | Data models for tasks and steps |
| `scripts/executor_utils.py` | Function execution utilities |
| `scripts/verb_utils.py` | Verb-aware classification helpers |
| `tasks/meraki-specialist/` | Specialist agent task definitions |
| `tasks/network-analyst/` | Analyst agent task definitions |

## Testing

- `python -m pytest tests/test_task_executor.py -v`
- `python -m pytest tests/test_task_registry.py -v`
- `python -m pytest tests/test_verb_utils.py -v`
