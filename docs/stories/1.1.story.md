...
- [x] **T1: Create `scripts/server.py` — FastAPI application skeleton** (AC: 1, 9, 10)
  - [x] T1.1: Create FastAPI app instance with title="CNL", version from `scripts.__version__`
  - [x] T1.2: Add CORS middleware allowing `localhost:5173` (Vite dev) and `localhost:3141`
  - [x] T1.3: Mount settings_routes router from `scripts.settings_routes` (Story 1.4)
  - [x] T1.4: Add static file serving for `frontend/dist/` (graceful skip if dir doesn't exist)
  - [x] T1.5: Create `run_server()` function using uvicorn with host=`127.0.0.1`, port from settings
  - [x] T1.6: Add startup event: log server version, port, and loaded routes
  - [x] T1.7: Add shutdown event: clean up any active connections

- [x] **T2: Implement health & status endpoints** (AC: 2)
  - [x] T2.1: `GET /api/v1/health` → return `{status: "ok", version: str, uptime_seconds: float}`
  - [x] T2.2: `GET /api/v1/status` → return connection status for Meraki, AI provider, N8N (calls SettingsManager to check)
  - [x] T2.3: Track server start time for uptime calculation

- [x] **T3: Implement profile endpoints** (AC: 7)
  - [x] T3.1: Create `scripts/profile_routes.py` with FastAPI APIRouter prefix `/api/v1/profiles`
  - [x] T3.2: `GET /api/v1/profiles` → call `auth.list_profiles()` via `asyncio.to_thread()`, return profile names
  - [x] T3.3: `GET /api/v1/profiles/{name}` → call `auth.load_profile(name)`, return masked profile info
  - [x] T3.4: `POST /api/v1/profiles/{name}/activate` → update settings.meraki_profile via SettingsManager
  - [x] T3.5: Handle `CredentialsNotFoundError` → 404, `InvalidProfileError` → 400

- [x] **T4: Implement discovery endpoints** (AC: 3)
  - [x] T4.1: Create `scripts/discovery_routes.py` with FastAPI APIRouter prefix `/api/v1/discovery`
  - [x] T4.2: `POST /api/v1/discovery/full` → body `{client_name: str}`, call `full_discovery()` via `asyncio.to_thread()`, return DiscoveryResult as JSON
  - [x] T4.3: `GET /api/v1/discovery/snapshots?client={name}` → call `list_snapshots(client_name)`, return snapshot paths with timestamps
  - [x] T4.4: `GET /api/v1/discovery/snapshots/{id}` → call `load_snapshot(path)`, return DiscoveryResult as JSON
  - [x] T4.5: `POST /api/v1/discovery/compare` → body `{old_path: str, new_path: str}`, call `compare_snapshots()`, return diff
  - [x] T4.6: Use Pydantic models for all request/response validation

- [x] **T5: Implement config endpoints** (AC: 4)
  - [x] T5.1: Create `scripts/config_routes.py` with FastAPI APIRouter prefix `/api/v1/config`
  - [x] T5.2: `POST /api/v1/config/ssid` → body with network_id, ssid params, call `configure_ssid()` via `asyncio.to_thread()`
  - [x] T5.3: `POST /api/v1/config/firewall` → body with network_id, rule params, call `add_firewall_rule()` via `asyncio.to_thread()`
  - [x] T5.4: `POST /api/v1/config/acl` → body with network_id, ACL params, call `add_switch_acl()` via `asyncio.to_thread()`
  - [x] T5.5: `POST /api/v1/config/vlan` → body with network_id, VLAN params, call `create_vlan()` via `asyncio.to_thread()`
  - [x] T5.6: `POST /api/v1/config/rollback` → body with network_id, backup info, call `rollback_config()` via `asyncio.to_thread()`
  - [x] T5.7: All config endpoints call `backup_config()` automatically before applying changes
  - [x] T5.8: Return `ConfigResult` with status, details, and backup path

- [x] **T6: Implement workflow endpoints** (AC: 5)
  - [x] T6.1: Create `scripts/workflow_routes.py` with FastAPI APIRouter prefix `/api/v1/workflows`
  - [x] T6.2: `GET /api/v1/workflows?client={name}` → call `list_workflows(client_name)`, return workflow list
  - [x] T6.3: `POST /api/v1/workflows` → body with workflow type + params, call appropriate `create_*()` function, return workflow JSON
  - [x] T6.4: `GET /api/v1/workflows/templates` → return list of available workflow template names
  - [x] T6.5: `POST /api/v1/workflows/from-template` → call `create_*()` using template type, return workflow + import instructions

- [x] **T7: Implement report endpoints** (AC: 6)
  - [x] T7.1: Create `scripts/report_routes.py` with FastAPI APIRouter prefix `/api/v1/reports`
  - [x] T7.2: `GET /api/v1/reports/{client}` → call `list_reports(client_name)`, return report paths
  - [x] T7.3: `GET /api/v1/reports/{client}/{filename}` → serve HTML file with `FileResponse`
  - [x] T7.4: `POST /api/v1/reports/{client}/generate` → call `generate_discovery_report()` via `asyncio.to_thread()`, return path

- [x] **T8: Implement global error handling** (AC: 8)
  - [x] T8.1: Create exception handlers for `CredentialsNotFoundError` → HTTP 404
  - [x] T8.2: Create exception handlers for `InvalidProfileError` → HTTP 400
  - [x] T8.3: Create exception handlers for `WorkflowError`, `WorkflowValidationError` → HTTP 422
  - [x] T8.4: Create catch-all handler for `meraki.exceptions.APIError` → HTTP 502 with Meraki error detail
  - [x] T8.5: Create generic Exception handler → HTTP 500 with safe error message (no internal stacktrace in response)
  - [x] T8.6: All error responses follow schema: `{error: str, detail: str, code: str}`

- [x] **T9: Add dependencies to requirements.txt** (AC: 1)
  - [x] T9.1: Add `fastapi>=0.115.0`, `uvicorn[standard]>=0.30.0` to requirements.txt

- [x] **T10: Write unit tests** (AC: all)
  - [x] T10.1: `tests/test_server.py` — Test app creation, CORS headers, static file serving
  - [x] T10.2: Test health endpoint returns correct schema
  - [x] T10.3: Test profile endpoints with mocked auth module
  - [x] T10.4: Test discovery endpoints with mocked discovery module
  - [x] T10.5: Test config endpoints with mocked config module
  - [x] T10.6: Test workflow endpoints with mocked workflow module
  - [x] T10.7: Test report endpoints with mocked filesystem
  - [x] T10.8: Test error handlers produce correct HTTP status codes
  - [x] T10.9: Test settings routes are mounted and reachable
  - [x] T10.10: Ensure all tests pass: `pytest tests/test_server.py -v`
...
- Test run: pytest tests/test_server.py -v (23 tests passed)
- All tests passed on first complete run after fixes

### Completion Notes List
1. **Server Architecture**: FastAPI app with modular router design - each feature domain gets its own route file
2. **Async Boundary Pattern**: All sync module calls wrapped with `asyncio.to_thread()` to maintain async context
3. **Error Handling**: Global exception handlers for all custom exceptions plus Meraki API errors
4. **Testing Strategy**: Comprehensive unit tests using TestClient with mocked dependencies
5. **CORS Configuration**: Restricted to localhost for security (dev: 5173, prod: 3141)
6. **Static File Serving**: Gracefully handles missing frontend/dist directory
7. **Settings Integration**: Successfully integrates Story 1.4 settings routes
8. **Test Coverage**: 23 tests covering health, profiles, discovery, config, workflows, reports, settings, and error handlers

### File List
```
scripts/server.py                  # Main FastAPI app (320 lines)
scripts/profile_routes.py          # Profile management endpoints (128 lines)
scripts/discovery_routes.py        # Discovery endpoints (164 lines)
scripts/config_routes.py           # Configuration endpoints (208 lines)
scripts/workflow_routes.py         # Workflow endpoints (232 lines)
scripts/report_routes.py           # Report endpoints (138 lines)
tests/test_server.py               # Comprehensive test suite (530 lines)
```

Total: 7 files created, ~1,720 lines of production code + tests
