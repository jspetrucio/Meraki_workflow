...
Um **squad** é um time de agentes de IA especializados que trabalham juntos em um domínio. Por exemplo, o squad `@copy` tem agentes baseados em Gary Halbert, Eugene Schwartz, Dan Kennedy - cada um com sua especialidade em copywriting.
...
**Exemplos:**
- Copywriting: Gary Halbert, Eugene Schwartz
- Marketing: Seth Godin, Russell Brunson
- Negócios: Peter Drucker, Jim Collins
...
| **Exemplo** | Gary Halbert | gary-halbert.md |
...
Se você está no repositório mmos, já está instalado. Basta ativar:
```
@squad-creator
```
...
O Squad Creator foi feito para o ecossistema AIOS/mmos. Para usar em outros projetos, você precisaria adaptar.
...
- ✅ Copywriting: Halbert + Schwartz + Kennedy
- ❌ Misto: Halbert (copy) + Drucker (gestão) + Ramsey (finanças)
...
mkdir -p materials/gary-halbert/

# Coloque PDFs, transcrições, etc
cp boron-letters.pdf materials/gary-halbert/
cp entrevista.txt materials/gary-halbert/

# Indique no comando
*create-squad copywriting --materials ./materials/
```
...
```bash
cp squads/copy/agents/gary-halbert.md ~/meus-agents/
```
...
*update-mind gary_halbert --sources ./novos-materiais/
...
*create-agent todd-brown --squad copy
```
...
```bash
rm squads/copy/agents/todd-brown.md
```
...
cp -r squads/copy/ ~/backups/copy-backup/

# Ou zipar
zip -r copy-backup.zip squads/copy/
```
